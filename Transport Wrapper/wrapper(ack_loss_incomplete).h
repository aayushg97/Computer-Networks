...
*/
void update_window(int *max_window, int *window, int *ssthresh, int recvbuffer_space, int flag){
	switch(flag){
		case 0: 
...
void rate_control(int sockfd){
	...
	int window = INITIAL_WINDOW_SIZE, max_window = INITIAL_WINDOW_SIZE;
	int out_bytes = 0;
	...
	while(1){
		...
		*/
		while(out_bytes<window && ((baseptr+1)%BUFFER_SIZE)<end){
			...
			if(out_bytes>max_window)
				max_window = out_bytes;
			flag = 1;	
		}
		...
		*/
		...
		if(timeout==0){
			...
			if((ack.seq_no>prev_seq && ack.seq_no-prev_seq<=max_window) || (ack.seq_no<prev_seq && MAX_SEQ-prev_seq+ack.seq_no <= max_window)){
				...
				*/
				...
				if(triple_dupack==1){
					...
					*/
					...
					dupack = 0;
					triple_dupack = 0;
					update_window(&max_window, &window, &ssthresh, ack.recvbuffer_space, 1);			// update window parameters
				}
				else{
					/*
						If ack of regular packet is received
					*/
					if((ack.seq_no>=curr_seq && ack.seq_no-curr_seq<=max_window)){
						baseptr = currptr;
						curr_seq = (prev_seq + 1)%MAX_SEQ;
					}
					update_window(&max_window, &window, &ssthresh, ack.recvbuffer_space, 0);			// update window parameters
				}
			...
			dupack = 0;
			triple_dupack = 0;
			update_window(&max_window, &window, &ssthresh, window, 2);
		}
...
void recvbuffer_handle(int sockfd){
	...
	while(1){
		...
		*/
		...
		if(data.seq_no==curr_seq){
			/*
				If expected data is received, send ack and add data to buffer
			*/
			printf("Ack sent with value %d\n",(curr_seq+data.length-1)%MAX_SEQ);
			if(drop_packet(drop_prob)==0)send_ack(sockfd, (curr_seq+data.length-1)%MAX_SEQ, temp, dataaddr, datalen);
