...
/*
	Structure to store ICMP payload
*/
typedef struct {
...
void updateStats(statistic *stats, double rtt){
	...
	temp = stats->n*pow(stats->mdev,2) + stats->n * pow(stats->avg, 2);
	temp += pow(rtt, 2);

	stats->avg = (stats->avg * stats->n + rtt)/(stats->n+1);

	stats->mdev = sqrt((temp - ((stats->n+1) * pow(stats->avg, 2)))/(stats->n+1));
...
void setICMPHeader(struct icmphdr *icmp_header, int seq_no){
	icmp_header->type = 8;			// echo req
	icmp_header->code = 0;			// sub type
