...
void setIPHeader(struct iphdr *ip_header, char *dest, int sockfd, int seq_no){
	...
	ip_header->saddr = inet_addr(inet_ntoa(getSourceIp(sockfd)));
...
double ping(int sockfd, char *dest, struct sockaddr_in hostaddr, int seq_no){
	...
	sendto(sockfd, snd_packet, sizeof(struct iphdr) + sizeof(struct icmphdr) + sizeof(payload)/*ip_header->tot_len*/, 0, (struct sockaddr *)(&hostaddr), sizeof(struct sockaddr));

	//printf("sendto %s\n",strerror(errno));

	hostlen = sizeof(hostaddr);
	d = recvfrom(sockfd, recv_packet, sizeof(struct iphdr) + sizeof(struct icmphdr) + sizeof(payload), 0, (struct sockaddr *)(&hostaddr), &hostlen);
	...
	if(d<0){
		...
		temp = (char *)(&(ip_reply->saddr));
		printf("from %d.%d.%d.%d ",temp[0],temp[1],temp[2],temp[3]);
...
int main(int argc, char **argv){
	...
	temp = (char *)(&(hostaddr.sin_addr.s_addr));

	printf("PING %s (%d.%d.%d.%d) %ld(%ld) bytes of data.\n", argv[1], temp[0], temp[1], temp[2], temp[3], sizeof(payload), sizeof(struct iphdr) + sizeof(struct icmphdr) + sizeof(payload));
